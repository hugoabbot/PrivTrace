...
class DiscretizationData:
    ...
    def get_discrete_data(self, trajectory_set):
        grid = Grid(self.cc)
        grid.get_grid(trajectory_set)
        grid.set_up_state(trajectory_set)
        ...
        if self.cc.trajectory_number_to_generate < 0:
            self.cc.trajectory_number_to_generate = trajectory_set.trajectory_number
        return grid
